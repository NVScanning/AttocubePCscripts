...
class ASC500Scanner(ASC500Base):
    ...
    def getPositionsXYRel(self):
        ...
        positions = [xPos, yPos]
        #print(f"in the positioning{positions}")
        return positions
    
    def setPositionsXYRel(self, positions, old_positions):
        """
        ...
        if not (abs(positions[0] - currPos[0]) < 2e-9 and abs(positions[1] - currPos[1]) < 2e-9):
            ...
            # print("Positioning!!")
            # print("Getting the value from the ASC that we set:")
            # print(self.getParameter(self.getConst('ID_POSI_TARGET_X'),0)) #asc takes inputs in 10pm)
            
        return(positions)
    def setPositionsX(self, val):
        """
        Sets the scanners x and y position relative to the voltage origin as list.
        ...
        # the following checks whether the starting and target positions are the same. This is needed
        # to avoid triggering the start of a scan
        if not (abs(val - currPos[0]) < 2e-9) :
            self.setParameter(self.getConst('ID_POSI_TARGET_X'), val*1e11) #asc takes inputs in 10pm
    ...
    def setPositionsY(self, val):
        ...
        # the following checks whether the starting and target positions are the same. This is needed
        # to avoid triggering the start of a scan
        if not (abs(val - currPos[1]) < 2e-9) :
            self.setParameter(self.getConst('ID_POSI_TARGET_Y'), val*1e11) 
            self.setParameter(self.getConst('ID_POSI_GOTO'), 0)
            print(val)
            print(val*1e11)
    ...
    def getScanFieldCentre(self):
        ...
        """
        xPos = self.getParameter(self.
                                 
                                 
                                 st('ID_SCAN_OFFSET_X'))*1e-11
