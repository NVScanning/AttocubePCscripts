    ...
    def tip_scan(self):
        ...
        try:
           ...
           # Check if a measurement is already running       
            ...
            # Get the parameters from the control panel, Moves is taken as the maximum position, get_output shows the current position
            # So the tip scan goes from the current position up to the value in the caption
            total_X_length = float(self.my_app.Moves[0].get())-float(self.my_app.ANC.get_output(1))
            total_Y_length = float(self.my_app.Moves[1].get())-float(self.my_app.ANC.get_output(2))
                        
            # Get the step size from the caption of the window
            delta_X = float(self.my_app.Steps[0].get())
            delta_Y = float(self.my_app.Steps[1].get())               
            ...
            self.x_array = np.arange(x0, total_X_length, stepsize[0])
            self.y_array = np.arange(y0, total_Y_length, stepsize[1])
            print(self.x_array)
            print(self.y_array)
            ...
            self.im.set_extent([x0 , total_X_length , y0, total_X_length])
